...
```java
PersonUpsertResult p = bf.people().upsert("erp-1042", "app-91", PersonUpsert.builder()
        .name("Paula Reis")
        .document("529.982.247-25")
        .build());
p.getDocument();    // "52998224725"
p.getMergedInto();  // "app-77" se o CPF já era de outra ficha; null se não
```
...
- **`.document(null)` (ou `.clear("document")`) NÃO apaga** o CPF (e `document` não é campo do `clear`). Omitir é o mesmo que "não mexe".
...
No lote: o mesmo `.document(...)` no `PersonUpsert` do `PersonBatchItem`.
